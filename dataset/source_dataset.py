...
class SourceDataset(SemanticDataset):
    def __init__(self, cfg, mode):
        ...
        self.tgt_dataset_name = cfg.DATASET_TARGET.TYPE
        
        self.use_dgt = cfg.DATASET_SOURCE.get("USE_DGT", False)
        if self.use_dgt:
            
            self.src_total_beams = cfg.DATASET_SOURCE.DGT.total_beams
            self.tgt_total_beams = cfg.DATASET_TARGET.DGT.total_beams
            
            if self.src_total_beams != self.tgt_total_beams:
                self.src_beam_label_path = cfg.DATASET_SOURCE.DGT.beam_label_path
        ...
        super().__init__(cfg.DATASET_SOURCE, mode) 
