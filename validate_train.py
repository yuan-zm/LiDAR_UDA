...
class validater:
    def __init__(self, 
        ...
        self.logger = logger
        self.dataset_name = self.cfg.DATASET_SOURCE.TYPE if domain == 'source' else self.cfg.DATASET_TARGET.TYPE

        from dataset.test_dataset import TestDataset
        self.test_dataset = TestDataset(self.cfg, domain, 'test')  # [gen_pselab | test]
    ...
    def rolling_predict(self, net_G, old_G, c_iter, src_centers=None, domain='src'):
        ...
        tqdm_loader = tqdm(self.test_dataloader,
                           ...
                           ncols=100, desc='validating **{}: {}**'.format(self.domain, self.dataset_name))
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(tqdm_loader):
                ...
                out_dict = net_G(val_G_in)
                val_logits_1 = out_dict['sp_out'].F
                val_logits_1 = val_logits_1[batch_data['inverse_map']]

                if self.cfg.MEAN_TEACHER.use_mt:
                    val_tea_dict = old_G(val_G_in)
                    val_logits_tea = val_tea_dict['sp_out'].F
